...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		If(Test-Path -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{CF1FCC73-CF68-4ED7-B976-9876AA6CFAE9}"){
			Write-Log -Message "Attempting to run EOIR installer..." -Source 'Installation' -LogType 'CMTrace'
			$exitCode = Execute-Process -Path "$dirFiles\Ansys_STK_Enterprise_v12.4.0\STKEOIR_v12.4.0\install.exe" -Parameters '/s /v"/qn"' -PassThru
